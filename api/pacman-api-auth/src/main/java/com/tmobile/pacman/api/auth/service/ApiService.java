...
public class ApiService {
	...
	public Map<String, Object> loginProxy(final UserClientCredentials credentials) {
		...
		try {
			requestBodyUrl = "password=".concat(URLEncoder.encode(credentials.getPassword(), "UTF-8")).concat("&grant_type=password&username=").concat(URLEncoder.encode(credentials.getUsername(), "UTF-8"));
	...
	private Map<String, Object> generateAccessToken(String requestBodyUrl, String clientId) {
		...
		try {
			...
			try {
				String clientCredentialsValue = authRepository.getClientAuthorization(clientId);
				clientCredentials = "Basic " + clientCredentialsValue;
	...
	public Map<String, Object> login(UserLoginCredentials credentials) {
		System.out.println("activeAuth==============>"+activeAuth);
