...
public class LdapAuthServiceImpl implements AuthService {
	...
	/**
	 * Adds user defauls group information to User Principle
	 * @param userDetails
	 * @return
	 *//*
	private Map<String, Object> addUserDefaultAssetGroup(final Map<String, Object> userDetails) {
		if(Boolean.parseBoolean(userDetails.get("success").toString())) {
			try {
				String defaultAg = getDefaultAssetGroupForUser(userDetails.get("userId").toString().toLowerCase());
					userDetails.put("defaultAssetGroup", defaultAg);
					userDetails.put("userRoles", authRepository.getUserRoleDetailsOld(userDetails.get("userId").toString().toLowerCase()));
			} catch(Exception exception){
				log.error("Exception in fetching defaultAssetGroup and recentlyViewedAG:: "+exception.getMessage());
				exception.printStackTrace();
				userDetails.put("defaultAssetGroup", StringUtils.EMPTY);
				userDetails.put("userRoles", Lists.newArrayList());
			}
		}
		return userDetails;
	}
*/
	/**
	 * Get user default asset group
	 * @param string
	 * @return
	 */
/*	private String getDefaultAssetGroupForUser(String userId) {
...
/*	public static void main(String[] args) throws InterruptedException {
			...
			LdapAuthenticatorService authenticator = new LdapAuthenticatorService();
			authenticator.setup();
			Map<String, Object> userDetails = authenticator.authenticateAndGetUserDetails("", "");
