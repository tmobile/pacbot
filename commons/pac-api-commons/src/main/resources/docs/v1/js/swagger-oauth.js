function handleLogin() {
    ...
    for (window.swaggerUi.api && window.swaggerUi.api.info && (appName = window.swaggerUi.api.info.title),
    ...
    p = 0; p < e.length; p++)
        ...
        t.description && ($.map(o, function(e, o) {
            return o
        }).length > 1 ? str += '<br/><span class="api-scope-desc">' + t.description + " (" + t.OAuthSchemeKey + ")</span>" : str += '<br/><span class="api-scope-desc">' + t.description + "</span>"),
        str += "</label></li>",
        popup.append(str);
    var r = $(window)
      , s = r.width()
            ...
            if (i.hasOwnProperty(h) && l.indexOf(h) != -1) {
                ...
                if ("oauth2" !== i[h].type || !g || "implicit" !== g && "accessCode" !== g) {
                    if ("oauth2" === i[h].type && g && "application" === g) {
                        var w = i[h];
                        return window.swaggerUi.tokenName = e(w) || tokenName,
                    ...
                    if (i[h].grantTypes) {
                        ...
                        for (var f in c)
                            if (c.hasOwnProperty(f) && "implicit" === f) {
                                var w = c[f];
                                w.loginEndpoint.url;
                                r = w.loginEndpoint.url + "?response_type="+tokenOrIdToken,
                    ...
                    var w = i[h];
                    r = w.authorizationUrl + "?response_type=" + ("implicit" === g ? tokenOrIdToken : "code"),
                    window.swaggerUi.tokenName = e(w) || tokenName,
...
window.onOAuthComplete = function(e, o) {
    if (e)
        if (e.error) {
        	...
        	var url = window.location.origin+"/api/auth/user/authorize";
            var n = e["id_token"];
